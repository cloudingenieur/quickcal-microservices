...
public class GatewayApplicationTests {
	...
	static void registerResourceServerIssuerProperty(DynamicPropertyRegistry registry) {
		...
		registry.add("spring.cloud.gateway.routes[0].uri",
				() -> "http://localhost:8060");
		registry.add("spring.cloud.gateway.routes[0].id", () -> "booking-service");
		registry.add("spring.cloud.gateway.routes[0].predicates[0]", () -> "Path=/bookings/v1/**");
	}
	...
	void shouldBeRedirectedToLoginPage() {
		webTestClient.get().uri("/bookings/v1/customers")
				.exchange()
